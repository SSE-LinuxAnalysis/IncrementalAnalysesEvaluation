...
for file in diffs/*.diff; do
    ...
    /usr/bin/time -v -o ./time/reference/time-$(echo "$file" | sed "s/.*\///").log java "-Xms${JVM_MIN_HEAP}" "-Xmx${JVM_MAX_HEAP}" -jar KernelHaven.jar configuration-reference.properties & disown
done
